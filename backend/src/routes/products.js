...
router.get("/", async (request, response, next) => {
  try {
    ...
    if (category && !validCategories.has(category)) {
      return response
        .status(400)
        .json({ error: "category must be women, men, or kid." });
    }
    if (!validSorts.has(sort)) {
      return response
        .status(400)
        .json({ error: "sort must be featured, price_asc, or price_desc." });
    }
    ...
    return response.json(
      products.map((product) => serialiseProduct(product, getBaseUrl(request))),
    );
  } catch (error) {
    return next(error);
  }
  ...
  try {
    ...
    if (
      ...
      return response
        ...
        .json({
          error:
            "Enter a name, category, valid image URL, current price, and an original price equal to or higher than the current price.",
        });
    }
    ...
    return response
      .status(201)
      .json({ product: serialiseProduct(product, getBaseUrl(request)) });
  } catch (error) {
    return next(error);
  }
});
    ...
    try {
      ...
      if (
        ...
        return response
          ...
          .json({
            error:
              "Enter a valid current price and an original price equal to or higher than the current price.",
          });
      }
      ...
      if (!product)
        return response.status(404).json({ error: "Product not found." });
      return response.json({
        product: serialiseProduct(product, getBaseUrl(request)),
      });
    } catch (error) {
      return next(error);
    }
